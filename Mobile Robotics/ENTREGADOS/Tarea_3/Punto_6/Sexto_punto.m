...
%variable de activación del control de velocidad
control_vel=0;
...
%fprintf('error_ang: %f \n',distancia_goal(i));
if(abs(error_angular(i))<=1e-3)
control_vel=1;

distancia_goal(i) = sqrt((x_goal-x(i))^2+(y_goal-y(i))^2);
end
if(distancia_goal(i)<=0.01 && control_vel == 1)
    control_vel=0;
    punto=punto+1;
    if(punto == 35)
        break
