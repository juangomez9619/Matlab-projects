...
for idx = 1:numel(t)
    ...
    if distancias(1,1)<= 0.4 || distancias(1,2)<= 0.3 || distancias(1,3)<= 0.3 || distancias(1,4)<= 0.22 || distancias(1,5)<= 0.2    
        %control angular
        vP(idx) = (vp_km1+f0*distancia_goal(idx)+f1*d_km1+f2*d_km2)*0.95;
